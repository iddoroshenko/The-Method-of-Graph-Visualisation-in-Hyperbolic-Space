    ...
    implements H3RenderList
{
	...
	if (INCLUDE_TREE_LINKS && m_graph.checkLinkVisible(link))
	{
		...
		/*
		////// iddoroshenko test
		//Integer.toString(sourceNode);
		try(FileWriter writer = new FileWriter("nodesCoord.txt", true)){
		writer.append(Integer.toString(sourceNode));
		writer.append(' ');
		//Double.toString()
		writer.append(Double.toString(m_source.x));
		writer.append(' ');
		writer.append(Double.toString(m_source.y));
		writer.append(' ');
		writer.append(Double.toString(m_source.z));
		writer.append('\n');
		writer.append(Integer.toString(targetNode));
		writer.append(' ');
		writer.append(Double.toString(m_target.x));
		writer.append(' ');
		writer.append(Double.toString(m_target.y));
		writer.append(' ');
		writer.append(Double.toString(m_target.z));
		writer.append('\n');
		writer.flush();
		}
